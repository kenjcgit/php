...
switch ($action) 
{
	...
	case 'ResetBadge':
		@mysql_query("UPDATE members SET memBadgeCount=0 WHERE memId='$memId'");
	...
	Case 'EditNotificationSettings':		
		...
		if($retStr=="")
		{
			...
			if($i==0)
			{
					...
					$membersSQL="UPDATE members SET  `memNewEventPush`='$memNewEventPush', `memGeneralPush`='$memGeneralPush', `memActivityReminderPush`='$memActivityReminderPush', `memEventReminderPush`='$memEventReminderPush', `memPush`='$memPush', `memCreatedDate`='$memCreatedDate' WHERE memId=".$memId;
					$resusers=mysql_query($membersSQL);	
					$result=getMember($memId);					
					$result[0]["status"]='true';					
			}
